...
class AdapterLayer(nn.Module):
    def __init__(self, 
                 dim: int, rank: int, num_experts: int = 4, top_k: int=2, expert_layer: nn.Module=SelfAttention, stage_depth: int=1,
...
class MoCEIR(nn.Module):
    def __init__(self,
                ...
                rank_type="constant",
                topk=1,
                expert_layer=SelfAttention, #OptimizedSelfAttention
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        random.seed(seed)
        # opt = train_options()
        model = MoCEIR(rank=2, num_blocks=[2, 2, 4, 4], num_dec_blocks=[1, 2, 4], levels=4, dim=8,
                       num_refinement_blocks=2, with_complexity=True, complexity_scale="max",
                       stage_depth=[1,1,1], heads=[1,2,4,8], depth_type="constant", rank_type="spread",
                        num_experts=4, topk=1)
        # print(model)
        model.eval()
        from ptflops import get_model_complexity_info
        macs, params = get_model_complexity_info(model, (3, 720, 1280), as_strings=True,
